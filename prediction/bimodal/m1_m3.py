    ...
    for path in paths:
        for h5_name in tqdm(os.listdir(path), leave=False):
            with h5py.File(os.path.join(path, h5_name), 'r') as h5:
                ...
                Y.append(y)
    return np.array(X_properties, dtype=np.float32), np.array(X_ch, dtype=np.float32), np.array(Y, dtype=np.float32)
...
def main():
    ...
    if len(sys.argv) != 7:
        raise ValueError(
            'usage: python m1_m3.py {partitions root} {training partitions} {validation partitions} {testing partitions} {convex hull model path} {output path}\nexample: python main.py /data 135 2 4 ./ch_model.pt ./out')
    root, training_partitions, validation_partitions, testing_partitions, ch_model_path, output_path = sys.argv[
        1:]
    ...
    x_properties_val, x_ch_val, y_val = get_raw_data(val_paths)
    x_properties_test, x_ch_test, y_test = get_raw_data(test_paths)
    ...
    fused_yhat_val = np.mean(
        np.stack((properties_yhat_val, ch_yhat_val)), axis=0)
    ...
    fused_yhat_test = np.mean(
        np.stack((properties_yhat_test, ch_yhat_test)), axis=0)
    ...
    print(df.head())
    df.to_csv(os.path.join(output_path, 'm1m3.csv'), index=False)
