...
def main():
    ...
    if len(sys.argv) != 8:
        raise ValueError(
            'usage: python main.py {partitions root} {training partitions} {validation partitions} {testing partitions} {pil model path} {convexhull model path} {output path}\nexample: python main.py /data 2 4 ./pil_model.pt ./ch_model.pt ./out')
    ...
    x_properties_train, _, _, y_train = get_raw_data(train_paths)
    x_properties_val, x_pil_val, x_ch_val, y_val = get_raw_data(val_paths)
    x_properties_test, x_pil_test, x_ch_test, y_test = get_raw_data(test_paths)
    ...
    x_properties_train_balanced = x_properties_train[np.append(f_ix, undersampled_nf_train_ix)]
    ...
    properties_yhat_val = properties_model.predict_proba(x_properties_val)[:, 1]
    with torch.no_grad():
        pil_yhat_val = pil_model(x_pil_val).cpu().detach().numpy().squeeze()
        ch_yhat_val = ch_model(x_ch_val).cpu().detach().numpy().squeeze()
    fused_yhat_val = np.mean(np.stack((properties_yhat_val, ch_yhat_val, pil_yhat_val)), axis=0)
    ...
    properties_yhat_test = properties_model.predict_proba(x_properties_test)[:, 1]
    with torch.no_grad():
        pil_yhat_test = pil_model(x_pil_test).cpu().detach().numpy().squeeze()
        ch_yhat_test = ch_model(x_ch_test).cpu().detach().numpy().squeeze()
    fused_yhat_test = np.mean(np.stack((properties_yhat_test, ch_yhat_test, pil_yhat_test)), axis=0)
    ...
    print(df.head())
    df.to_csv(os.path.join(output_path, 'm1m2m3.csv'), index=False)
