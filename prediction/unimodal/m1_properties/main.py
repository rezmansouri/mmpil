...
def get_raw_data(paths):
    ...
    X, Y = [], []
    for path in paths:
        for h5_name in tqdm(os.listdir(f'{path}')[:10], leave=False):
...
def main():
    ...
    if len(sys.argv) != 5:
        raise ValueError(
            'usage: python main.py {partitions root} {training partitions} {testing partitions} {output path}\nexample: python main.py /data 135 4 ./output')
    root, training_partitions, testing_partitions, output_path = sys.argv[1:]
    ...
    model_1.save('./tsf')
    model_1.load_from_path('./tsf.zip')
    ...
    df.to_csv(os.path.join(output_path, 'stats.csv'), index=False)
