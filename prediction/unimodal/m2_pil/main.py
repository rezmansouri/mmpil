...
def main():
    if len(sys.argv) != 7:
        raise ValueError(
            'usage: python main.py {partitions root} {training partitions} {validation partitions} {testing partitions} {model ix} {output path}\nexample: python main.py /data 135 2 4 1 ./output')
    root, training_partitions, validation_partitions, testing_partitions, model_ix, output_path = sys.argv[1:]
    ...
    if model_ix == 1:
        model = models.Model_1().to(DEVICE)
    elif model_ix == 2:
        model = models.Model_2().to(DEVICE)
    elif model_ix == 3:
        model = models.Model_3().to(DEVICE)
    else:
    ...
    torch.save(best_model_state_dict, os.path.join(
        output_path, f'best_{best_model_epoch}.pth'))

    torch.save(model.state_dict(), os.path.join(
        output_path, f'last_{n_epochs}.pth'))
    ...
    df.to_csv(os.path.join(output_path, 'stats.csv'))
